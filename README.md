...
```
    ...
    <ipython-input-1-7ae7c4cc7990> in <module>()
    ----> 1 csv = os.path.join('santhosh.csv')
          2 movie_df = pd.read_csv(csv,encoding='latin-1')
...
```
    ...
    <ipython-input-2-6ec2f5ff81d4> in <module>()
    ----> 1 movie_df = movie_df.dropna(subset = ['Overview'])
          2 movie_df['roi']=(movie_df['Revenue']-movie_df['Budget'])/movie_df['Budget']
          3 movie_df = movie_df[movie_df.roi<30]
          4 compound_list = []
          5 for overview in movie_df['Overview']:
...
```
    ...
    <ipython-input-3-d4d9b04eced2> in <module>()
          1 
    ----> 2 g = sns.jointplot("roi", "Sentiment Score", data=movie_df, kind="reg", color="r", size=7)
          3 plt.savefig('Sentiment Score Plot')
...
```
    ...
    <ipython-input-4-199865a6b08a> in <module>()
          1 #Form Dataframe grouped by production company
    ----> 2 movie_df_sum=movie_df.groupby(['Production Company']).sum()
          3 movie_df_sum['ROI']=(movie_df_sum['Revenue']-movie_df_sum['Budget'])/movie_df_sum['Budget']
          4 movie_df_sum=movie_df_sum.sort_values(['ROI'],ascending = False)
          5 movie_df_sum = movie_df_sum[movie_df_sum.ROI<150]
